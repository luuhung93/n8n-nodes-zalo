			...
			try {
				if (resource === 'zaloPoll') {
					...
					if (operation === 'createPoll') {
						...
						const optionInputType = this.getNodeParameter(
							'optionInputType',
							i,
							'list',
						) as string;
						...
						if (optionInputType === 'list') {
							try {
								const pollOptionsCollection = this.getNodeParameter(
								...
								throw new NodeOperationError(
									this.getNode(),
									'Lỗi xử lý các lựa chọn: ' +
										(error.message || 'Lỗi không xác định'),
								);
							}
						} else if (optionInputType === 'text') {
							const optionsString = this.getNodeParameter(
								'optionsString',
								i,
								'',
							) as string;
						...
						const expiredTime =
							this.getNodeParameter('expiredTime', i, null) !== null
								? new Date(
										this.getNodeParameter('expiredTime', i) as string,
									).getTime() || 0
								: 0;
						...
						const hideVotePreview = this.getNodeParameter(
							'hideVotePreview',
							i,
							false,
						) as boolean;
						const isAnonymous = this.getNodeParameter(
							'isAnonymous',
							i,
							false,
						) as boolean;
						...
						this.logger.info(
							`Create poll with parameters: ${JSON.stringify(createPollData)}`,
						);
